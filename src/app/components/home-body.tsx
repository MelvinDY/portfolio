...
/** Ink target colours for the manifesto scrub. The stagger, easing and scrub
 *  of that tween are untouched; only the colour it resolves changed, because
 *  the original #F2EAE0 is invisible on a light ground. */
const INK = { fill: '#14120F', acid: '#C13E00' }
...
const ABOUT_INK: InkWord[] = [
  ...
  { t: 'room' }, { t: 'go' }, { t: '“oh”,', acid: true }, { t: 'so' }, { t: 'now' },
...
const Ink = ({ words, fill, acidFill }: { words: InkWord[]; fill: string; acidFill: string }) => (
  <p className="ink" data-ink>
    {words.map((w, i) => (
      <span
        key={i}
        className={`iw${w.acid ? ' iw-acid' : ''}`}
        data-fill={w.acid ? acidFill : fill}
      >
        {w.t}{' '}
      </span>
    ))}
...
export default function HomeBody() {
  ...
  /* The wrapper paints its own ground, but body keeps the dark page colour, so
     an overscroll bounce flashes dark behind a light page. Toggling one class
     fixes it and touches nothing the animation depends on. */
  useEffect(() => {
    document.body.classList.add('home-light-page')
    return () => document.body.classList.remove('home-light-page')
  }, [])
        ...
        <section id="about" className="sec">
          <div className="wrap">
            ...
            <div className="ab2-grid">
              ...
              <Ink words={ABOUT_INK} fill={INK.fill} acidFill={INK.acid} />
            </div>
